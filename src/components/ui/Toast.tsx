		...
		<div
			...
			<div className='flex items-start gap-3 p-4'>
				...
				<div className='flex-1 min-w-0'>
					{toast.title && <h4 className='font-semibold text-sm text-zinc-900 dark:text-zinc-50 mb-0.5 leading-none'>{toast.title}</h4>}
					<p className='text-sm text-zinc-500 dark:text-zinc-400 leading-normal'>{toast.message}</p>
					{toast.action && (
						<button
							onClick={(e) => {
								e.stopPropagation();
...
export const ToastProvider: React.FC<{children: ReactNode}> = ({children}) => {
	...
	return (
		<ToastContext.Provider value={{showToast, hideToast}}>
			{children}
			<div className='fixed top-6 left-6 z-[100000] flex flex-col gap-3 pointer-events-none'>
