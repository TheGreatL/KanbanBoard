...
export default function Column({
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    await addTask(column.id, trimTitle, trimContent);
    setNewTaskTitle("");
    setNewTaskContent("");
    setIsAdding(false);
  };
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`${bgColorClass} w-80 shrink-0 flex flex-col rounded-2xl border border-zinc-200 dark:border-zinc-800 border-t-4 ${borderColorClass} transition-colors duration-300 h-max`}
    >
      <div
        {...attributes}
        {...listeners}
        tabIndex={undefined}
        className="p-4 flex items-center justify-between cursor-grab active:cursor-grabbing group border-b border-zinc-200/50 dark:border-zinc-800/50 relative"
      >
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {/* Visual indicator and color picker trigger */}
          ...
          {isEditingTitle ? (
            <form onSubmit={handleTitleSubmit} className="flex-1 min-w-0 mr-2">
                <input
                    autoFocus
                    value={editTitle}
             ...
             <h3 
               className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm truncate cursor-text hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50 rounded px-1.5 py-0.5 -ml-1.5 transition-colors"
               onClick={(e) => {
                   e.stopPropagation();
          ...
          <span className={`shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ml-1 transition-colors duration-300 ${badgeColorClass}`}>
            {tasks.length}
          </span>
        </div>
        <button
          ...
          onPointerDown={(e) => e.stopPropagation()}
          onMouseDown={(e) => e.preventDefault()}
          className="shrink-0 opacity-0 group-hover:opacity-100 text-zinc-400 hover:text-red-500 transition-opacity p-1 rounded-md hover:bg-zinc-200 dark:hover:bg-zinc-800"
        >
        ...
        {isAdding ? (
          <form onSubmit={handleAddTask} className="mt-1 flex flex-col gap-2">
            <input
              ...
              className="w-full px-3 py-2 text-sm bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-1 focus:ring-zinc-400 dark:focus:ring-zinc-600 transition-all text-zinc-900 dark:text-zinc-100 font-medium placeholder-zinc-400"
              placeholder="Task title…"
            />
            <textarea
              value={newTaskContent}
              onChange={(e) => setNewTaskContent(e.target.value)}
              onBlur={() => !newTaskTitle && !newTaskContent && setIsAdding(false)}
              onPointerDown={(e) => e.stopPropagation()}
              onKeyDown={(e) => {
              ...
              className="w-full px-3 py-2 text-sm bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-1 focus:ring-zinc-400 dark:focus:ring-zinc-600 transition-all text-zinc-900 dark:text-zinc-100 resize-none placeholder-zinc-400"
            ...
            <div className="flex gap-2 mt-2">
              <button
                type="submit"
                onPointerDown={(e) => e.stopPropagation()}
                className="px-3 py-1.5 text-xs bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 font-medium rounded-lg hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors cursor-pointer"
              >
                Add
              </button>
              <button
                type="button"
                onClick={() => { setNewTaskTitle(""); setNewTaskContent(""); setIsAdding(false); }}
                onPointerDown={(e) => e.stopPropagation()}
                className="px-3 py-1.5 text-xs text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors font-medium cursor-pointer"
              >
                Cancel
              </button>
            </div>
          ...
          <button
            ...
            onPointerDown={(e) => e.stopPropagation()}
            onMouseDown={(e) => e.preventDefault()}
            className="flex items-center gap-2 w-full p-2 text-sm text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-200/50 dark:hover:bg-zinc-800/50 rounded-xl transition-colors mt-1 cursor-pointer"
          >
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-zinc-900 dark:text-zinc-100 text-base">Delete Column</h2>
              <button
